//! Implementation for lustre node `FUZ` (see [FUZ](struct.FUZ.html)).
//!
...
impl Sys for FUZ {
  type Input = (
    Real, // svar_Input (FUZ.usr.Input)
    Real, // svar_Now (FUZ.usr.Now)
    Real, // svar_Kp (FUZ.usr.Kp)
    Real, // svar_Ki (FUZ.usr.Ki)
    Real, // svar_Kd (FUZ.usr.Kd)
    Real, // svar_Setpoint (FUZ.usr.Setpoint)
    Real, // svar_SampleTime (FUZ.usr.SampleTime)
  ) ;
  type Output = (
    Real, // svar_Output (FUZ.usr.Output)
  ) ;
/*  fn arity() -> usize { 7 }
